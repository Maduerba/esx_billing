					...
					if xPlayer.getMoney() >= amount then
						MySQL.Async.execute('DELETE FROM billing WHERE id = @id', {
							...
							if rowsChanged == 1 then
								...
								if xTarget then
									xTarget.showNotification(_U('received_payment', ESX.Math.GroupDigits(amount)))
									TriggerEvent('nexus-webhook:billing:pay_bill', {result[1].target, xTarget.name, xTarget.job.name, xTarget.job.grade_label, xTarget.identifier, xPlayer.name, xPlayer.identifier, result[1].label, amount})
								else
									TriggerEvent('nexus-webhook:billing:pay_bill', {result[1].target, 'Not found', '', '', result[1].sender, xPlayer.name, xPlayer.identifier, result[1].label, amount})
								end
							end
